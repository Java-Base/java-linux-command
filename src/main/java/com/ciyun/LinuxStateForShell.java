...
/**
 * 远程调用Linux shell 命令
 * @url		http://www.faceye.net/search/147249.html
 * 
 * @author 	Lian
 * @date	2016年5月5日
 * @since	1.0	
...
public class LinuxStateForShell {
			...
			if (command.equals(CPU_MEM_SHELL)) {
				...
				for (String line : strings) {
					...
					if (line.startsWith("CPU(S):")) {
						...
						try {
							memStr += line.split(":")[1].replace("TOTAL", "总计").replace("USED", "已使用").replace("FREE", "空闲").replace("BUFFERS", "缓存");
	...
	private static String disposeFilesSystem(String commandResult) {
		...
		return new StringBuilder().append("大小 ").append(size).append("G , 已使用").append(used).append("G ,空闲").append(size - used).append("G")
				.toString();
	}
...
//		Map<String, String> result = runDistanceShell(COMMANDS, "root", "@@123", "192.168.1.233");
		Map<String, String> result = runDistanceShell(COMMANDS, "root", "Ciyun@520", "112.74.74.201");
